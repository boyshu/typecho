...
<div id="comments">
  ...
  <div class="post-yiyan card"> <span class="text-muted letterspacing indexWords"><?php echo getYiyan();?></span> </div>
  <?php endif;?>
  <?php $this->comments('comment')->to($comments); ?>
  <?php if($this->allow('comment') && !$this->is('attachment')): ?>
  <?php if(allowcomment()):?>
  <div id="<?php $this->respondId(); ?>" class="respond">
    <div class="cancel-comment-reply"> <a id="cancel-comment-reply-link" data-href="<?php echo $this->parameter->parentContent['permalink'] . '#' . $this->parameter->respondId;?>" rel="nofollow" style="display:none;" onclick="return TypechoComment.cancelReply();">取消回复</a> </div>
    ...
    <div class="card response-form emoji">
      <form method="post" action="<?php $this->commentUrl() ?>" id="comment-form" role="form">
        ...
        <div class="form-item form-loginfo">欢迎您，<a target="_blank" data-no-instant href="<?php $this->options->profileUrl(); ?>" rel="nofollow">
          <?php $this->user->screenName(); ?>
          </a>，<a data-no-instant href="<?php $this->options->logoutUrl(); ?>" title="退出登录" rel="nofollow"><i class="fa fa-sign-out"></i></a></div>
        <?php else: ?>
        <div id="form-user" class="<?php if ($this->remember('author', true)): ?> form-item-hide<?php endif; ?>">
          <div class="form-item">
            <label for="comment-author">昵称：</label>
            <input type="text" name="author" placeholder="*" id="comment-author" class="text" value="<?php $this->remember('author'); ?>" required />
          </div>
          <div class="form-item">
            <label for="comment-mail">邮箱：</label>
            <input type="email" placeholder="*" name="mail" id="mail" class="text" value="<?php $this->remember('mail'); ?>"<?php if ($this->options->commentsRequireMail): ?> required<?php endif; ?> />
          </div>
          <div class="form-item">
            <label for="comment-url">网址：</label>
            <input id="url" class="text" name="url" type="url" value="<?php $this->remember('url'); ?>" placeholder="" />
          </div>
        </div>
        <?php endif; ?>
        <?php if ($this->remember('author', true)&&!$this->user->hasLogin()): ?>
        <div class="form-item form-loginfo"> 将以 <a id="form-user-edit" href="###" title="更改身份信息">
          <?php $this->remember('author'); ?>
          </a> 的身份发表评论： </div>
        <?php endif; ?>
        <div class="form-item form-textarea">
          <div>
            <textarea rows="8" placeholder="<?php if($this->options->commentsRequireModeration && !$this->user->hasLogin()){ echo "（已开启评论审核模式，发表后不会立即显示）

";}?><?php if($this->options->inputyiyan == '1') {echo getYiyan();} else{echo $this->options->defaultcomment;}?>" name="text" id="comment-text" class="textarea" required ><?php $this->remember('text'); ?>
</textarea>
          </div>
        </div>
        <div class="form-emoji" id="form-emoji">
          <button id="smilies" class="smiles" type="button"> <i class="fa fa-smile-o"></i> </button>
            ...
            <div class="smiles-sidebar" style="display:none" id="smiles-sidebar">
              <div class="smiles-widget smiles-widget-tab" id="smiles-widget">
                ...
                <div class="smiles-widget-box">
                  <ul class="new-list">
                    <li> <?php echo outputsilies(0);?> <?php echo outputsilies(2);?> </li>
                  </ul>
                  <ul class="hot-list">
  ...
  <div class="box">
    <h3 class="box-label"><span class="label-left"></span><span class="box-name">
      <?php $this->commentsNum(_t('最新评论'), _t('最新评论'), _t('最新评论')); ?>
      </span></h3>
