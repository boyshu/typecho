...
<?php if(!empty($this->options->ismeaemail)){
  echo '<a href="'.fullurl($this->options->ismeaemail,0).'" target="_blank"><i class="fa fa-envelope"></i></a>';}?>
...
<?php if($this->options->istags == '1' && !$this->is('page','cross') && !$this->is('page','link')): ?>
...
<?php if($this->options->isrecommend == '1'): ?>
  <?php if (!$this->is('post')):$this->widget('Widget_Contents_Post_Recent')->to($recent); ?>
